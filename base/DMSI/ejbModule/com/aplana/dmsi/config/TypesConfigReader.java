...
public class TypesConfigReader extends ConfigReader<Type> {
     ...
     * (non-Javadoc)
     *
     * @see com.aplana.dmsi.config.ConfigReader#readConfigs(org.w3c.dom.Element)
     */
    @Override
	...
	Class<? extends DMSIObject> entityTypeClass = readEntityTypeClass(entityElement);
    ...
    private Class<? extends DMSIObject> readEntityTypeClass(Element entityElement) {
	Class<? extends DMSIObject> typeClass = readClassFromSubElement(
    ...
    private ObjectId getTemplateId(Class<? extends DMSIObject> typeClass) {
    ...
    private Class<? extends DMSIObject> readEntityMTypeClass(Element entityElement) {
	return readClassFromSubElement(entityElement, "mTypeClass");
    }

    private Class<? extends DMSIObject> readClassFromSubElement(Element parent,
	    String elementName) {
	Element subElement = XmlUtils.getSingleSubElement(parent, elementName);
	if (subElement != null && !"".equals(subElement.getTextContent())) {
	    return initializeDMSIObjectType(subElement.getTextContent());
	}
	return null;
    }

    private Class<? extends DMSIObjectFactory> readFactory(Element configElement) {
	Element factoryElement = XmlUtils.getSingleSubElement(configElement,
		"factory");

	if (factoryElement != null
		&& !"".equals(factoryElement.getTextContent())) {
	    return ReflectionUtils.initializeClass(DMSIObjectFactory.class,
