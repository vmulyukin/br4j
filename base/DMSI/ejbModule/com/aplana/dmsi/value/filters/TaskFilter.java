...
public class TaskFilter implements CollectionFilter, Parametrized {

    private static final String STATES_PARAM = "states";

    private Map<Class<? extends Task>, List<Long>> states = new HashMap<Class<? extends Task>, List<Long>>();
	...
	if (STATES_PARAM.equals(key)) {
		...
		throw new ConfigurationException(
			"Value of "
				+ STATES_PARAM
				+ " should be like '<Task class>:<state1_id>,<state2_id>,...,<staten_id>'");
	    }
	    String className = classStatesPair[0];
	    Class<? extends Task> clazz = ReflectionUtils.initializeClass(
	...
	for (Iterator<?> iter = collection.iterator(); iter.hasNext();) {
	    ...
	    if (states.containsKey(task.getClass())
		    && !states.get(task.getClass()).contains(((Task) task).getState())) {
		iter.remove();
	    }
	}
    }
