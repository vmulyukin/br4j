...
(function(){
	...
	var size = function(widget, dim){
		...
		// record child's size
		if(newSize){
			// if the child returned its new size then use that
...
dojo.declare("dojox.grid.Selection", null, {
	...
	addToSelection: function(inIndex){
		...
		if(this.selected[inIndex]){
			...
			if(this.onCanSelect(inIndex) !== false){
				...
				this._endUpdate();
				var d =  dojo.query('.dijitCheckBox', rowNode)[0];
				if(d){
					dijit.byNode(d).setChecked(true);
				}
			}
		}
	},
	...
	deselect: function(inIndex){
		...
		if(this.selected[inIndex]){
			...
			var d =  dojo.query('.dijitCheckBox', rowNode)[0];
			if(d){
				dijit.byNode(d).setChecked(false);
			}
		}
	},
...
(function(){
	...
	dojo.declare('dojox.grid._Grid',
		...
		doheaderclick: function(e){
			...
			var view = this;//TODO Разобраться основательно, почему после сортировки увеличивается высота строк.
			setTimeout(function(){
				if (window.jQuery){
					var pane = jQuery('.dojoxGridContent', view.domNode);
					var h = 0;
					pane.children().each(function(a,b){h=h+jQuery(b).outerHeight()});
					jQuery('.dojoxGridContent' ,view.domNode).height(h);
					jQuery('.dojoxGridMasterView' ,view.domNode).height(h);
					console.log(h);
				}
			},100);
		},
...
=====*/
	...
	sort: function(){
		...
		this.prevSelection = this.selection.getSelected();//custom recheck after sorting
		this._refresh();
		//custom recheck after sorting
		if(this.prevSelection.length && this.prevSelection.length>0){
			this.selection.deselectAll();
			for(var i=0; i<this.rowCount; i++){
				for(var j=0; j<this.prevSelection.length; j++){
					if(this.getItem(i)._0 == this.prevSelection[j]._0){
