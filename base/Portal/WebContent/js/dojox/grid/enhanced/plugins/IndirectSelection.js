...
this.grid=_1;
this.grid.selection.toggleAllTrigerred = false;
dojo.connect(_1.layout,"setStructure",dojo.hitch(_1.layout,this.addRowSelectCell));
dojo.connect(this.grid.views,'render',dojo.hitch(this, function(){
	var v = _1.views.views[0];
	if(v){
		var _44 = v.getHeaderCellNode(0);
		if(_44 && _44.childNodes[0]){
			 var checkBox = new dijit.form.CheckBox({
			        name: "checkBox",
			        value: "agreed",
			        checked: this.grid.selection.toggleAllTrigerred,
					grid: _1,
			        onChange: function(){ 
						if(this.checked){
							this.grid.selection.toggleAllTrigerred = true;
							this.grid.selection.selectRange(0,this.grid.rowCount-1);
						} else {
							this.grid.selection.toggleAllTrigerred = false;
							this.grid.selection.deselectAll();
						}
					}
			    }, _44.childNodes[0]).startup();
		}
	}
}));
...
this.needFormatNode(_d,_e);
	...
	var _f2 = this.grid.selection.selected[_10];
	if(this.widgetMap[this.view.id] && this.widgetMap[this.view.id][_10]){
		this.widgetMap[this.view.id][_10].setChecked(_f2);
	}
	this.formatNode(_f2,_10);
