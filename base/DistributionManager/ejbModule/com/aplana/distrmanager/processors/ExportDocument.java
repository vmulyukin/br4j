...
public class ExportDocument extends ProcessCard {
    ...
    private static final String MESSAGE_GOST_ERROR = "jbr.DistributionManager.ExportDocument.notFound.messageGOSTCard";
    private static final String SAVE_XML_ERROR = "jbr.DistributionManager.ExportDocument.errorSaveXml";
    private static final String NONCRITICAL_ERROR = "jbr.DistributionManager.ExportDocument.noncriticalError";
    private static final String SUCCESSFUL = "jbr.DistributionManager.ExportDocument.SUCCESSFUL";
    ...
    private static final String WRITE_RESULT_ERROR_CRITICAL = "jbr.DistributionManager.ExportDocument.write.result Rollback!";
    private static final String SAVE_CARD_ERROR_CRITICAL = "jbr.DistributionManager.ExportDocument.save.card Rollback!";
    private static final String CHANGE_STATE_ERROR_CRITICAL = "jbr.DistributionManager.ExportDocument.change.status Rollback!";
    private static final String CHANGE_STATE_ERROR = "jbr.DistributionManager.ExportDocument.change.status";
    private static final String ERROR_CRITICAL = "jbr.DistributionManager.ExportDocument::doProcess Critical error! Rollback!";
	...
	public Object process() throws DataException {
		try {
			try {
				...
				xmlLetter = CreateLetter.instance(serviceBean).handle(msgGOSTWrap, stateCardWrap, elmCardWrap);
				...
				throw
					new DataException(wre);
			}
			catch (SaveCardException sce) { // ����������� ������
				logError(currentCard, UNSUCCESSFUL_SAVE, sce);
				throw
					new DataException(sce);
			}
			catch (ChangeStateException cse) { // ������������� ������
				// ��������� ��� � ������ "������������" � �������������� ��������� ���������.
				logError(currentCard, CHANGE_STATE_ERROR, cse);
				UtilsWorkingFiles.deleteDirectory(file);
				elmCardWrap.byPrepareNotSent(getJdbcTemplate()); // ����������� ������
				stateCardWrap.setResultProcessing(cse.getCause().getCause().getMessage()); // ����������� ������
				UtilsWorkingFiles.saveCard(stateCardWrap.getCard(), serviceBean); // ����������� ������
			}
