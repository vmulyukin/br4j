...
public class OperationsOnLetter {
	...
	private Letter createLetter() throws Exception {
		ObjectId gostCardId = elm.findCardMessageGOST();
		MessageGOST msgGost = new MessageGOST(serviceBean);
		msgGost.init(gostCardId);
		...
		addressee.setGuid(msgGost.getRecipientUid());
		addressee.setName(msgGost.getRecipient());
		addressee.setValue(null);
		letter.setAddressee(addressee);

		Sender sender = new Sender();
		sender.setGuid(msgGost.getUidSender());
		sender.setName(msgGost.getFullNameSender());
