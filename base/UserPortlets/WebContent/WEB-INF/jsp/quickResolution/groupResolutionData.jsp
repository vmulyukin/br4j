...
<script type="text/javascript">
	...
	var <portlet:namespace/>_docsGroup = ${requestScope.docsGroup};
	...
	function <portlet:namespace/>_buildDocsGroup(){
		var table = dojo.byId('<portlet:namespace/>__docsGroup');
		<portlet:namespace/>_docsGroup.forEach(function(doc){
...
<form id="<portlet:namespace/>_form" method="post" action="<portlet:actionURL/>">
	...
	<div class="resolution">
        ...
        <hr/>
        <span style="margin: 2px; display: block;"><fmt:message key="header.groupresolution.selected.docs"/></span>
		<div style="height: auto;" class="contentDiv">
			<table id="<portlet:namespace/>__docsGroup">
			</table>
		</div>
		<hr/>
		<!-- begin ЭЦП -->
            ...
            <div class="resolution_buttons">
                ...
                <div>
					<a  class="button blue"href="#"
						onclick="<portlet:namespace/>_submitForm('<%= QuickResolutionPortlet.ACTION_DONE %>')"  >На исполнение</a>
                </div>
            </div>
...
<script type="text/javascript">
	...
	dojo.addOnLoad(function() {
		...
		<portlet:namespace/>_refreshControls();
		<portlet:namespace/>_buildDocsGroup();
