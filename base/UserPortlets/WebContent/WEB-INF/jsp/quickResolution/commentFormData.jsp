...
<form id="<portlet:namespace/>_form" method="post"  action="<portlet:actionURL/>">
	...
	<input id="<portlet:namespace/>_signature" name="<%=QuickResolutionPortlet.PARAM_SIGNATURE%>" type="hidden" class="attrString" value="" />
        ...
        <div class="single_column">
			<textarea style="width: 600px" name="<%=QuickResolutionPortlet.FIELD_TEXT_RESOLUTION%>" id="<portlet:namespace/>_<%=QuickResolutionPortlet.FIELD_TEXT_RESOLUTION%>" placeholder="<fmt:message key="label.decisionText"/>" class="resolution_text" id="<portlet:namespace/>_text_res"><c:out value="${sessionBean.resolutionText}"/></textarea>
			<br>
			<%if(renderRequest.getAttribute(QuickResolutionPortlet.PARAM_DECISION).equals(QuickResolutionPortlet.PARAM_DECISION_DECLINE)){%>
        		<div style="display:none">
			<%}else{%>
				<div>
			<%}%>
        
            <label><fmt:message key="label.uploadAttachment"/>:</label>
            <input class="filePathInput" readonly type="text" id="<portlet:namespace/>_filePath" />
            
			<button dojoType="dijit.form.Button" type="button" style="height: 22px; margin-left: 5px; ">
			...
			<button dojoType="dijit.form.Button" style="height: 22px; margin-left: 5px; " 
...
<script type="text/javascript">
	...
	function <portlet:namespace/>_validate() {
		var resText = dojo.byId('<portlet:namespace/>_<%=QuickResolutionPortlet.FIELD_TEXT_RESOLUTION%>');

		if(resText && resText.value.length > 0) {
			return true;
		}
		alert('<fmt:message key="message.emptyResolution"/>');
		return false;
	}
