...
<%@page import="com.aplana.dbmi.roleadmin.RoleAdminPortlet"%>
		...
		<div style="float:right; clear: both;" id="dialogButtons">
			...
			<button id="${yesBtnId}" dojoType="dijit.form.Button" type="button">
			    ...
			    <script type="dojo/method" event="onClick" args="evt">
						submitForm('<%=RoleAdminPortlet.DELETE_ROLE_ACTION%>');
				</script>		
			</button>
	...
	<div class="buttonPanel" style="float: right">
		<ul>
			<c:set var="onClickCallback">submitForm('<%=RoleAdminPortlet.BACK_ACTION%>')</c:set>
			<btn:button onClick="${onClickCallback}" textKey="role.admin.close.btn" />

			<c:set var="onClickCallback">submitForm('<%=RoleAdminPortlet.OPEN_EDIT_MODE_ACTION%>')</c:set>
			<btn:button tooltipKey="tool.edit"
				onClick="${onClickCallback}"
				icon="ico_edit"/>
			<c:set var="onClickCallback">dijit.byId('${confirmationDialogId}').show()</c:set>
			<btn:button onClick="${onClickCallback}" textKey="role.admin.delete.btn"/>
		</ul>
	</div>