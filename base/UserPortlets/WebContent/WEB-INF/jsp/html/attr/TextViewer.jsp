...
<%  TextAttribute attr = (TextAttribute) request.getAttribute(JspAttributeEditor.ATTR_ATTRIBUTE);
	...
	if (attr != null && attr.getValue() != null) {
		rows = attr.getValue().split("\n").length;
		if (rows < 1) rows = 1;
	}
	if (rows > 7) {
	rows = 7;
%>
	<textarea style="border-color:white;background-color:white;color:black;"
 		class="attrText"
...
%>
			<div>
<%				for (String part : StringEscapeUtils.escapeHtml(attr.getValue()).split("\n")) {
%>
					<%= part %>
					<br>
<%				}
