...
--%>
<%@page import="com.aplana.dbmi.model.SearchIntegerCheckedAttribute"%>
...
<%
	SearchIntegerCheckedAttribute attr = (SearchIntegerCheckedAttribute) request.getAttribute(JspAttributeEditor.ATTR_ATTRIBUTE); 
...
<div class="textCheckedStyle">
	<!-- input dojoType="dijit.form.ValidationTextBox"  style="width:94%; float:left;"
	 name="${attrHtmlId}_inputInteger"
	 id="${attrHtmlId}_inputInteger"
	 value=""
	 onChange="javascript: editorEventManager.notifyValueChanged('${attrHtmlId}_inputInteger', this.value);"		 
	/-->
	<input dojoType="dijit.form.ValidationTextBox"  style="width:94%; float:left;"
	 name="${attrHtmlId}_inputInteger"
	 id="${attrHtmlId}_inputInteger"
	 value="<%= attr.getValue() > 0 ? attr.getValue() : "" %>"
	 onChange="javascript: editorEventManager.notifyValueChanged('${attrHtmlId}_inputInteger', this.value);"
	 regExp="[0-9]+" required="false" invalidMessage="<fmt:message key="search.invalid.number"/>"
...
editorEventManager.registerAttributeEditor('${attrHtmlId}_inputString', '${attrHtmlId}_inputInteger', false, dojo.byId('${attrHtmlId}_inputInteger').value);
...
function ${attrHtmlId}_ChangeStrictText(checkboxElement) {
	...
	var widget = dojo.byId('${attrHtmlId}' + '_inputInteger');
	
	if(${attrHtmlId}_toolTip==null){
		${attrHtmlId}_toolTip = new dijit.Tooltip({
	        connectId: ["${attrHtmlId}_inputInteger"],
	        label: ""
	    });
	}
