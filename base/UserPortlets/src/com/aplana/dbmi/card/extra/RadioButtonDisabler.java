...
public class RadioButtonDisabler extends DisablerBaseBuilder{
	...
	protected void addDisabler(Attribute attr, String valueAttrId, String values, boolean valuesToEnable, StringBuilder stringBuilder) {
		...
		String functionName = attrHtmlId + "_from_" + valueAttrId.replace(".", "_") + "_radioButtonDisabler";
		String getControlExpression = "dijit.byId('" + attrHtmlId + "')";

		stringBuilder.append("function " + functionName + "(attrCode, attrHtmlId, isInline, value, param) { ");
		stringBuilder.append("for(var i in " + attrHtmlId + "_radioButtons) {");
		stringBuilder.append("	var btn = " + attrHtmlId + "_radioButtons[i];");
		if (values != null) {
			stringBuilder.append("btn.setDisabled(");
			addCheckValuesExpression(values, valuesToEnable, stringBuilder);
			stringBuilder.append(");");
		}
		else {
			stringBuilder.append("btn.setDisabled(value == null || typeof(value) == 'undefined' || value == '');");
		}
		stringBuilder.append("}}");
