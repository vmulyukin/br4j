...
<div class="reportheader">
	<c:choose>
		...
		<c:otherwise>
			<c:set var="logActionFormId"><portlet:namespace/>_cardStateForm</c:set>
	...
	<c:if test="${not viewMode}">
		<c:choose>
			<c:when test="${not empty command.selectedObject.id}">
				<spring:message code="partition.editObject" var="logActionFormPartition"/>
			</c:when>
			<c:otherwise>
				<spring:message code="partition.createNewObject" var="logActionFormPartition"/>
			</c:otherwise>
		</c:choose>
		...
		<c:set var="logActionFormObjectPath" value="selectedObject"/>
