...
public class ListUsersByGroup extends ChildrenQueryBase {

	@Override
	public Object processQuery() throws DataException {
		
		final ObjectId groupId = getParent();
		
		if(groupId.getId() == null) return null;
		...
		getJdbcTemplate().query(
				"SELECT p.person_id, p.person_login, p.full_name, p.email, \n" +	// 1 - 4
					...
					"p.card_id \n" +												// 9
				"FROM person p join person_role_group pr on pr.person_id = p.person_id \n" +
				"WHERE pr.group_code = ?",
				new Object[] { groupId.getId() },
		...
		for (Person person : result) {
			ChildrenQueryBase subQuery = getQueryFactory().getChildrenQuery(Person.class, Group.class);
			subQuery.setParent(person.getId());
