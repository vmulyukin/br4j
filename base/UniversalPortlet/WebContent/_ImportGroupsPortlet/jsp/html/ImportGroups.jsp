...
<%@page import="com.aplana.dbmi.importgroupsportlet.*"%>
<%@page import="com.aplana.dbmi.importbaseportlet.*"%>
<%@page import="com.aplana.dbmi.grouplistportlet.*"%>
...
<fmt:setBundle basename="com.aplana.dbmi.importgroupsportlet.nl.GroupImportPortlet" scope="request"/>

<%
BaseImportPortletSessionBean sessionBean = (BaseImportPortletSessionBean)renderRequest.getPortletSession().getAttribute(GroupsImportPortlet.SESSION_BEAN);
...
ResourceBundle bundle = ResourceBundle.getBundle("com.aplana.dbmi.importgroupsportlet.nl.GroupImportPortlet", request.getLocale());
if (sessionBean==null){
...
/*if( message != null) {
	...
	message = renderRequest.getParameter(GroupListPortlet.MSG_PARAM_NAME);
}*/
%>
	<c:set var="selectedVal"
		value="<%=GroupsImportPortlet.SELECTED_VAL%>" />
	<c:set var="unSelectedVal"
		value="<%=GroupsImportPortlet.UNSELECTED_VAL%>" />
...
function onToggleButtonClick(element, dependElement, hiddenElementName) {
	...
	<c:if test="<%= renderRequest.getParameter(GroupsImportPortlet.SHOW_WARNING_MESSAGE) != null%>">
	...
	<script>
		...
		function addRow() {
			...
			cell.innerHTML ='<input type="file" name="<%=GroupsImportPortlet.FILENAME_PARAM%>"/>';
			var input = cell.childNodes[0];

			cell = row.insertCell(1);
			cell.innerHTML = '<input type="text" name="<%=GroupsImportPortlet.MATERIALNAME_PARAM%>" style="visibility:hidden;"/>';
