...
<%@page import="com.aplana.dbmi.importrolesportlet.*"%>
<%@page import="com.aplana.dbmi.importbaseportlet.*"%>
<%@page import="com.aplana.dbmi.rolelistportlet.*"%>
<%@page import="com.aplana.dbmi.universalportlet.*"%>
...
<fmt:setBundle basename="com.aplana.dbmi.importrolesportlet.nl.RoleImportPortlet" scope="request"/>

<%
BaseImportPortletSessionBean sessionBean = (BaseImportPortletSessionBean)renderRequest.getPortletSession().getAttribute(RolesImportPortlet.SESSION_BEAN);
...
ResourceBundle bundle = ResourceBundle.getBundle("com.aplana.dbmi.importrolesportlet.nl.RoleImportPortlet", request.getLocale());
if (sessionBean==null){
	PortletURL backURL = renderResponse.createActionURL();
	/*backURL.setParameter(RoleListPortlet.ACTION_FIELD, RoleListPortlet.BACK_ACTION); 
...
<%
	...
	String startMessage = bundle.getString("form.upload.file.title");
	String loadMessage = bundle.getString("form.upload.file.title.users");
	String backURL;
	if (sessionBean!=null){
...
/*if( message != null) {
	...
	message = renderRequest.getParameter(RoleListPortlet.MSG_PARAM_NAME);
}*/
%>
	<c:set var="selectedVal"
		value="<%=RolesImportPortlet.SELECTED_VAL%>" />
	<c:set var="unSelectedVal"
		value="<%=RolesImportPortlet.UNSELECTED_VAL%>" />
...
function showLoadScreen() {
	...
	dojo.query('#dbmiLoadingSplash').children().children()[0].innerHTML = '<%=loadMessage%>';
}
...
function onToggleButtonClick(element, dependElement, hiddenElementName) {
	...
	<c:if test="<%= renderRequest.getParameter(RolesImportPortlet.SHOW_WARNING_MESSAGE) != null%>">
	...
	<script>
		...
		function addRow() {
			...
			cell.innerHTML ='<input type="file" name="<%=RolesImportPortlet.FILENAME_PARAM%>"/>';
			var input = cell.childNodes[0];

			cell = row.insertCell(1);
			cell.innerHTML = '<input type="text" name="<%=RolesImportPortlet.MATERIALNAME_PARAM%>" style="visibility:hidden;"/>';
