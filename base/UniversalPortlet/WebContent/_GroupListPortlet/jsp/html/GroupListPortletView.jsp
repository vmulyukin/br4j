...
--%>
<%@page session="false" contentType="text/html" pageEncoding="UTF-8" import="java.util.*,javax.portlet.*,com.aplana.dbmi.universalportlet.*, com.aplana.dbmi.grouplistportlet.*" %>
...
<fmt:setBundle basename="com.aplana.dbmi.grouplistportlet.nl.GroupListPortletResource" scope="request"/>
<%
	...
	PortletMessage message = (PortletMessage)renderRequest.getPortletSession().getAttribute(GroupListPortlet.MSG_PARAM_NAME, PortletSession.APPLICATION_SCOPE);
	renderRequest.getPortletSession().removeAttribute(GroupListPortlet.MSG_PARAM_NAME, PortletSession.APPLICATION_SCOPE);
%>
		...
		<div style="float:right; clear: both;" id="dialogButtons">
			...
			<button id="${yesBtnId}" dojoType="dijit.form.Button" type="button">
			    ...
			    <script type="dojo/method" event="onClick" args="evt">
						submitForm_action('<%= GroupListPortlet.DELETE_GROUP_ACTION %>', this);
				</script>		
			</button>
			...
			<button id="${noBtnId}" dojoType="dijit.form.Button" type="button">
				...
				<script type="dojo/method" event="onClick" args="evt">
					document.UniversalPortletForm.<%= GroupListPortlet.MI_DELETE_ENTITY_ID %>.value = "";
...
<script type="text/javascript">
	...
	function submitForm_action(action, elem) { 
		...
		document.UniversalPortletForm.<%= GroupListPortlet.ACTION_FIELD %>.value = action;
		document.UniversalPortletForm.submit();
	}
	function submitForm_delete(elem, entityId) {
		document.UniversalPortletForm.<%= GroupListPortlet.MI_DELETE_ENTITY_ID %>.value = entityId;
    ...
    <div class="toolbar">
    	<div class="buttonPanel">
			...
			<%	
				...
				urlParams.put(GroupListPortlet.MI_CREATE_ENTITY, "true");
				PortletURL backURL = renderResponse.createRenderURL();
				backURL.setParameters(renderRequest.getParameterMap());
				urlParams.put(GroupListPortlet.BACK_URL_FIELD, URLEncoder.encode(backURL.toString(), "UTF-8"));
				final String createGroupURL = portletService.generateLink("dbmi.Group", "dbmi.Group.w.Group", urlParams, renderRequest, renderResponse);
			...
			<%  if (sessionBean.isShowCreate() && sessionBean.isCreateAccessExists()) {%>
		 			    	<btn:button tooltipKey="tool.group.create"
										href="<%= response.encodeURL(createGroupURL) %>" 
										icon="ico_new"/>
			<%}%>
			<%if (editAccessExists&&sessionBean.isCanImport()&&sessionBean.isShowCreate()) {
				final PortletURL importGroupURL = renderResponse.createActionURL();
				importGroupURL.setParameter(GroupListPortlet.ACTION_FIELD, GroupListPortlet.IMPORT_GROUP_ACTION); 
				importGroupURL.setParameter(GroupListPortlet.BACK_URL_FIELD, URLEncoder.encode(backURL.toString(), "UTF-8"));%> 
				<btn:button tooltipKey="tool.import.groups.xls"
							href="<%= importGroupURL.toString() %>"
							icon="ico_xls_in"/>
			<%}%>
			<%
...
<input type="hidden" name="<%= GroupListPortlet.ACTION_FIELD %>" value="">
<input type="hidden" name="<%= GroupListPortlet.MI_DELETE_ENTITY_ID %>" value="">
</form>
</div>

<%
GroupListPortletTableDecorator decorator = new GroupListPortletTableDecorator(renderRequest, renderResponse);
...
<% 
	...
	if (columnValues != null && !columnValues.isEmpty()) {
		...
		params.put(GroupListPortlet.MI_OPEN_ENTITY_ID, entityId);
		params.put(GroupListPortlet.OPEN_FOR_EDIT_FIELD, "true");
		params.put(GroupListPortlet.BACK_URL_FIELD, URLEncoder.encode(backURL.toString(), "UTF-8"));
		String editLink = portletService.generateLink("dbmi.Group", "dbmi.Group.w.Group", params, renderRequest, renderResponse);
	%>
		<display:column>
		<fmt:message var="deleteTooltip" key="tool.group.delete" />
		...
		<display:column>
		<fmt:message var="editTooltip" key="tool.group.edit" />
